...
public abstract class AbstractDetailPane extends JPanel {
   ...
   /** Text prepended to constant pool hyperlinks. */
   public static final String BOOTSTRAP_METHOD_LINK_TEXT = "bootstrap #";
    ...
    protected void bootstrapMethodHyperlink(ExtendedJLabel value,
                                        ExtendedJLabel comment,
                                        int methodIndex) {

       value.setText(BOOTSTRAP_METHOD_LINK_TEXT + methodIndex);
       setupBootstrapMouseListener(value, methodIndex);
       value.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));

       if (comment != null) {
           comment.setToolTipText(comment.getText());
           BootstrapMethodsAttribute bmAttribute = (BootstrapMethodsAttribute) services.getClassFile().findAttribute(BootstrapMethodsAttribute.class);
           BootstrapMethodEntry method = bmAttribute.getBootstrapMethods()[methodIndex];
           String name = getConstantPoolEntryName(method.getAttributeNameIndex());
    ...
    private void setupBootstrapMouseListener(ExtendedJLabel value, int methodIndex) {

         MouseListener oldListener = (MouseListener)labelToMouseListener.get(value);
         if (oldListener != null) {
             value.removeMouseListener(oldListener);
         }
         MouseListener newListener = new BootstrapMethodHyperlinkListener(
                                         services,
                                         methodIndex);
